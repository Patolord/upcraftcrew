...
export default function DashboardPage() {
	...
	const _averageProgress = projects?.length
		? Math.round(
			...
			<View className="p-4 space-y-4">
				...
				<View className="flex-row flex-wrap gap-3">
					...
					<View className="flex-1 min-w-[45%] bg-white p-4 rounded-lg shadow">
						...
						<Text
							className={`text-3xl font-bold mt-1 ${netProfit >= 0 ? "text-green-600" : "text-red-600"}`}
						>
							${netProfit.toLocaleString()}
						</Text>
						<Text className="text-xs text-gray-400 mt-1">
							{totalExpenses > 0
								? `${Math.round((netProfit / totalIncome) * 100)}% margin`
								: "no expenses"}
						</Text>
					</View>
				</View>
