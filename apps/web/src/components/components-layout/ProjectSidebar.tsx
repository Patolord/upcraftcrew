...
export const ProjectSidebarDemo = ({ menuItems = [] }: ProjectSidebarProps) => {
	return (
		<>
			<input
				type="checkbox"
				...
				<div className="sidebar-menu relative flex h-full w-full flex-col py-3">
					...
					<div className="custom-scrollbar grow overflow-auto">
						...
						{menuItems.map((item) => {
							...
							if (!item.children) {
								return (
									<div key={item.id} className="mt-1 space-y-0.5 px-2.5">
										<Link href={item.url || "#"} className="menu-item group" {...(item.linkProp || {})}>
											{item.icon && <span className={`iconify ${item.icon} size-4`}></span>}
									...
									<div className="mt-1 space-y-0.5 px-2.5">
										{item.children.map((child) => (
											<Link 
												...
												{...(child.linkProp || {})}
											>
												{child.icon && <span className={`iconify ${child.icon} size-4`}></span>}
												<p className="grow">{child.label}</p>
												<span className="iconify lucide--chevron-right size-3.5 -translate-x-1 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-60"></span>
											</Link>
										))}
									</div>
