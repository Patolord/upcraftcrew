	...
	return (
		<>
			<div className={`dropdown ${dropdownClass ?? ""}`}>
				...
				>
					<span
						className={`iconify lucide--sun hidden size-4 group-data-[theme=light]/html:inline-block ${iconClass ?? ""}`}
					/>
					<span
						className={`iconify lucide--moon hidden size-4 group-data-[theme=dark]/html:inline-block ${iconClass ?? ""}`}
					/>
					<span
						className={`iconify lucide--monitor hidden size-4 group-[:not([data-theme])]/html:inline-block ${iconClass ?? ""}`}
					/>
				</div>
				<ul
					tabIndex={0}
