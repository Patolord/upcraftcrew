...
const LayoutBuilderPage = () => {
	...
	const [selectedTopbar, setSelectedTopbar] = useState<string>(
		topbars[0].title,
	);
	...
	const Sidebar = useMemo(
		() =>
			sidebars.find((sidebar) => sidebar.title === selectedSidebar)?.comp ??
			fallbackSidebar,
		[selectedSidebar],
	);
	...
	const Topbar = useMemo(
		() =>
			topbars.find((topbar) => topbar.title === selectedTopbar)?.comp ??
			fallbackTopbar,
		[selectedTopbar],
	);
	...
	const Footer = useMemo(
		() =>
			footers.find((footer) => footer.title === selectedFooter)?.comp ??
			fallbackFooter,
		[selectedFooter],
	);
	...
	return (
		<div className="size-full">
			<div className="flex">
				...
				/>
				<div
					id="layout-sidebar-hover"
					className="bg-base-300 h-screen w-1"
				></div>
				<div
					id="layout-sidebar"
				...
				<div className="flex h-screen min-w-0 grow flex-col overflow-auto">
					...
					<div id="layout-content">
						<PageTitle
							...
							items={[{ label: "Layout Builder", active: true }]}
						/>
						...
						<div className="bg-base-100 rounded-box border-base-200 mt-8 max-w-md border p-5 xl:mt-12 2xl:mt-16">
							...
							<p className="text-base-content/80 mt-1 text-sm">
								...
								<Link
									...
									href="/components/layouts/sidebar"
								>
									sidebar
								</Link>
								,{" "}
								<Link
									className="link link-hover text-primary"
									href="/components/layouts/topbar"
								>
									topbar
								</Link>{" "}
								and{" "}
								<Link
									className="link link-hover text-primary"
									href="/components/layouts/footer"
								>
									footer
								</Link>{" "}
								are available in the components section for easy access and
								customization.
							</p>
						</div>
					</div>
