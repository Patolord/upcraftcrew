...
export const RecentOrderTableRow = ({
	image,
	price,
	status,
	date,
	name,
}: IRecentOrderTableRow) => {
	return (
		<tr>
			<th>
				<input
			...
			<td className="flex items-center space-x-3 truncate">
				<img
					alt="order image"
					className="mask mask-squircle bg-base-200 size-7.5"
					src={image}
				/>
				<p>{name}</p>
			</td>
			...
			<td>
				{status == "delivered" && (
					<div className="badge badge-success badge-sm badge-soft">
						Delivered
					</div>
				)}
				{status == "on_going" && (
					<div className="badge badge-info badge-sm badge-soft">On Going</div>
				)}
				{status == "confirmed" && (
				...
				{status == "canceled" && (
					<div className="badge badge-error badge-sm badge-soft">Canceled</div>
				)}
				{status == "waiting" && (
					<div className="badge badge-secondary badge-sm badge-soft">
						Waiting
					</div>
				)}
			</td>
