...
import type { ITableData } from "./helper";

export const RenderCell = ({ cell }: { cell: Cell<ITableData, unknown> }) => {
	const { avatar, status, customerName, id, dateTime, amount, email } =
		cell.row.original;
	switch (cell.column.id) {
		case "id":
			return (
				<span className="text-base-content/70 font-mono text-xs uppercase">
					#{id}
				</span>
			);
		case "customerName":
