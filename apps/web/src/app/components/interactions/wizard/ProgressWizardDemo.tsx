				...
				<div className="grid grid-cols-4 items-center gap-3">
					{steps.map((step, index) => (
						...
						>
							...
							>
								<span
									className={`iconify lucide--check ${index + 1 >= currentStep ? "hidden" : ""}`}
								></span>
								<span className={`${index + 1 >= currentStep ? "" : "hidden"}`}>
									{index + 1}
								</span>
							</div>
