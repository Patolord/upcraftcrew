					...
					<div className="grid grid-cols-2 gap-4 p-4 sm:gap-6 sm:p-6 lg:grid-cols-4">
						{blendModes.map((mode, index) => (
							...
							>
								...
								></div>
								<p className="text-base-content/60 sm:t ext-sm text-xs">
									{mode}
								</p>
							</div>
						))}
						...
						{backdropFilters.map((filter, index) => (
							...
							>
								<div className="relative">
									...
									/>
									<div
										className={`rounded-box absolute start-1/2 top-1/2 size-20 -translate-1/2 bg-white/30 md:size-28 ${filter.class}`}
									></div>
								</div>
