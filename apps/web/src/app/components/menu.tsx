				...
				preview: (
					<div className="grid grid-cols-3 gap-2">
						{Array.from({ length: 9 }).map((_, i) => (
							<div
								key={i}
								className="bg-base-content/20 size-6 rounded-xs"
							></div>
						))}
					</div>
				),
					...
					<div className="w-3/4 space-y-2.5 sm:w-3/5">
						<div className="grid grid-cols-4 gap-2">
							{Array.from({ length: 4 }).map((_, i) => (
								<div
									key={i}
									className="border-base-content/30 h-4 rounded-xs border border-dashed"
								></div>
							))}
						</div>
						...
						<div className="grid grid-cols-3 gap-2">
							{Array.from({ length: 3 }).map((_, i) => (
								<div
									key={i}
									className="border-base-content/30 h-6 rounded-xs border border-dashed"
								></div>
							))}
						</div>
					</div>
						...
						<div className="border-base-300 mt-1.5 rounded-xs border p-1.5">
							...
							<div className="mt-2 grid grid-cols-7 gap-1">
								...
								{Array.from({ length: 14 }).map((_, i) => (
									<div
										key={i}
										className="bg-base-content/15 size-2 rounded-xs"
									></div>
								))}
			...
			},
			{
				...
				preview: (
					<div className="text-base-content/70 flex items-center gap-2.5">
						...
						<div className="border-base-300 w-18 rounded-xs border px-2 py-1 text-sm">
							45
						</div>
						<div className="bg-base-content/20 flex items-center justify-center rounded-full p-1.5">
				...
				preview: (
					<div className="border-base-300 w-3/4 rounded-xs border sm:w-1/2">
						<div className="text-base-content/70 border-base-300 flex items-center gap-2 border-b p-1.5 text-[10px]">
							<div className="bg-base-content/15 rounded-full px-1.5 py-0.5">
								Design
							</div>
				...
				preview: (
					<div>
						<svg className="size-24" viewBox="0 0 120 120">
							...
							>
								<path
									d=" M 0 0 L 0 -58 A 58 58 0 0 1 34.8 -49.3 Z "
